...
- **Telegram-уведомления** — агрегированные отчёты с настраиваемым интервалом, профитом в USD и балансами по каждому токену; адрес бота может быть ссылкой на блок-эксплорер через `scanner_url`
- **Мгновенные алерты об убытке** — опциональный `loss_alert_usd` для каждой сети: как только блок оказался убыточным, приходит отдельное сообщение со ссылками на транзакции
...
bots:
  ethereum:
    ...
    watched_address: '0x...'
    # scanner_url (опционально) — базовый URL блок-эксплорера сети. Делает адрес
    # бота в Telegram-уведомлениях кликабельной ссылкой.
    # Примеры: ethereum — https://etherscan.io/, arbitrum — https://arbiscan.io/
    scanner_url: 'https://etherscan.io/'
    # loss_alert_usd (опционально) — мгновенный алерт, как только P&L блока в этой
    # сети ниже -$loss_alert_usd. Не указывать — выключено, 0 — на любой убыток.
    loss_alert_usd: 1.0
...
`scanner_url` тоже опционален: если он задан, адрес бота в Telegram-сообщениях
становится кликабельной ссылкой на его страницу в блок-эксплорере (например,
`https://etherscan.io/address/0x...`); если не задан — адрес выводится обычным
...
### Мгновенные алерты об убытке

`loss_alert_usd` задаётся отдельно для каждой сети, рядом с `watched_address`
этого бота — так можно алертить на любой убыток в дешёвой L2 и только на крупный
в mainnet, а для ненужных сетей просто не указывать параметр.

Если он задан, каждый блок с транзакциями отслеживаемого адреса оценивается сразу
после чтения. Если его P&L (изменение балансов токенов минус газ) ниже
`-loss_alert_usd`, отдельное сообщение уходит немедленно — не дожидаясь следующего
отчёта по расписанию. В периодический отчёт блок при этом попадает как обычно.

Хеши транзакций становятся ссылками на блок-эксплорер, если задан `scanner_url`
(иначе выводятся моноширинным текстом). В блоке может быть несколько транзакций
бота — перечисляются все:
```
🚨 LOSS — ETHEREUM $-10.12
0x1234...5678
├ Block: 21500123
...
`loss_alert_usd: 0` шлёт алерт на любой убыток; большее значение (например, `5`)
отсекает шум от дешёвых зафейленных транзакций. USD-цены кешируются на минуту,
поэтому частые блоки не нагружают API CoinGecko.
